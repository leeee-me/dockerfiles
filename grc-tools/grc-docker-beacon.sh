...
C=`grc-docker.sh beaconreport | /usr/bin/jq ".[] | select(.cpid==\"$B\")"`
echo $C | jq -r 'keys_unsorted[] as $k | "\($k), \(.[$k])"' | grep -e ^cpid -e ^address -e ^status

echo
echo "=== Active beacon (be aware of flags) ==="
grc-docker.sh beaconstatus | jq -r '.active[] | keys_unsorted[] as $k | "\($k), \(.[$k])"' | grep -e ^active -e ^expired -e ^is_minei -e ^verification_code  -e ^timestamp -e ^address

echo
echo "=== Pending beacon (be aware of flags) ==="
grc-docker.sh beaconstatus | jq -r '.pending[] | keys_unsorted[] as $k | "\($k), \(.[$k])"' | grep -e ^active -e ^expired -e ^is_minei -e ^verification_code -e ^timestamp -e ^address

A=`grc-docker.sh beaconreport | /usr/bin/jq ".[] | select(.cpid==\"$B\") | .timestamp"`
d1=$A
d2=`date -d now +%s --utc`
echo
echo "==="
